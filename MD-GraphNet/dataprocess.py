...
def process_data(dataset):
    ...
    for i in range(len(names)):
        with open("../data/citedata/ind.{}.{}".format(dataset, names[i]), 'rb') as f:
    ...
    print(graph)
    test_idx_reorder = parse_index_file("../data/citedata/ind.{}.test.index".format(dataset))
    ...
    print(features)
    f = open('../data/{}/{}.edge'.format(dataset, dataset), 'w+')
    ...
    np.savetxt('../data/{}/test20'.format(dataset, dataset), np.array(test_idx_range), fmt='%d')
